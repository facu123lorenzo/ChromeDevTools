...
namespace MasterDevs.ChromeDevTools.Protocol.Chrome.LayerTree
{
	...
	public class ReplaySnapshotCommand: ICommand<ReplaySnapshotCommandResponse>
	{
		...
		/// <summary>
		/// Gets or setsThe first step to replay from (replay from the very start if not specified).

		/// </summary>
		[JsonProperty(NullValueHandling = NullValueHandling.Ignore)]
		public long? FromStep { get; set; }
		/// <summary>
		/// Gets or setsThe last step to replay to (replay till the end if not specified).

		/// </summary>
		[JsonProperty(NullValueHandling = NullValueHandling.Ignore)]
		public long? ToStep { get; set; }
		/// <summary>
		/// Gets or setsThe scale to apply while replaying (defaults to 1).
